...
class YOLO(object):
    ...
    #   检测图片
    #---------------------------------------------------#
    def detect_image(self, image, classes, crop = False, count = False):
        #---------------------------------------------------#
        #   计算输入图片的高和宽
        #---------------------------------------------------#
        ...
        if classes is None:
            result_dict = dict.fromkeys(self.class_names)
        else:
            result_dict = dict.fromkeys(classes)
        for i in range(self.num_classes):
            if classes is not None:
                if not(self.class_names[i] in classes):
                    continue
            num = np.sum(top_label == i)
            if num > 0:
                result_dict[self.class_names[i]] = str(num)
            # if num > 0:
                # print(self.class_names[i], " : ", num)
        ...
        for i, c in list(enumerate(top_label)):
            ...
            if classes is not None:
                if not(predicted_class in classes):
                    continue
            box             = top_boxes[i]
            score           = top_conf[i]
